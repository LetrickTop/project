...
class TrafficSniffer:
    def __init__(self, interface="eth0", db_path="traffic.db"):
        ...
        # Счётчики для `count`, `srv_count`, `dst_host_count`, `dst_host_srv_count`
    ...
    def packet_handler(self, packet):
        ...
        if packet.haslayer(scapy.IP):
            ...
            dst_bytes = src_bytes  # Без обратного трафика сложно выделить

            # Определение `service` по порту
            service = "Unknown"
            ...
            # Подсчет частоты появления IP и сервисов
